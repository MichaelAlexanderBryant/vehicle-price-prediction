...
I found that the most important features for the SVR model were `Seller_Type`, `Present_Price`, and `Year`. When the vehicles were sold by the owner rather than by a dealer, the selling price was significantly less. The other two features are self-explanatory where the current average selling price and year of the vehicle is highly-correlated positively with the sale price. `Kms_Driven` is not a significant factor, but this is likely due to being correlated with `Year`.
